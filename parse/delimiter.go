...
func (t *Tree) scanDelims(ctx *InlineContext) *delimiter {
	...
	if 0 < startPos {
		...
		if (t.Context.ParseOption.EditorWYSIWYG || t.Context.ParseOption.EditorIR || t.Context.ParseOption.EditorSV || t.Context.ParseOption.ProtyleWYSIWYG) && editor.Caret == string(tokenBefore) {
			// 跳过插入符位置向前看
