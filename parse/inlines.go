...
func (t *Tree) walkParseInline(node *ast.Node) {
	...
	if ast.NodeParagraph == typ || ast.NodeHeading == typ || ast.NodeTableCell == typ {
		...
		if ast.NodeParagraph == typ {
			if nil == tokens {
				if ast.NodeListItem != node.Parent.Type || t.Context.ParseOption.EditorWYSIWYG || t.Context.ParseOption.EditorIR || t.Context.ParseOption.EditorSV {
		...
		if t.Context.ParseOption.GFMAutoLink && !t.Context.ParseOption.EditorWYSIWYG && !t.Context.ParseOption.EditorIR && !t.Context.ParseOption.EditorSV && !t.Context.ParseOption.ProtyleWYSIWYG {
