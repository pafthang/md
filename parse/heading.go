...
func SetextHeadingStart(t *Tree, container *ast.Node) int {
	...
	if t.Context.ParseOption.GFMTable {
		...
		//
		// 前两行可以解析出一个只有一个单元格的表。
		// Empty list following GFM Table makes table broken https://github.com/b3log/md/issues/9
...
func (t *Tree) parseATXHeading() (ok bool, markers, content []byte, level int) {
	...
	var startCaret bool
	if (t.Context.ParseOption.EditorWYSIWYG || t.Context.ParseOption.EditorIR || t.Context.ParseOption.EditorSV || t.Context.ParseOption.ProtyleWYSIWYG) && bytes.HasPrefix(tokens, editor.CaretTokens) {
	...
	var inCaret bool
	if (t.Context.ParseOption.EditorWYSIWYG || t.Context.ParseOption.EditorIR || t.Context.ParseOption.EditorSV) && bytes.Contains(tokens, []byte("#"+editor.Caret+"#")) {
		tokens = bytes.ReplaceAll(tokens, editor.CaretTokens, nil)
		inCaret = true
	}
	...
	var endCaret bool
	if (t.Context.ParseOption.EditorWYSIWYG || t.Context.ParseOption.EditorIR || t.Context.ParseOption.EditorSV) && bytes.HasPrefix(tokens[level:], []byte(" "+editor.Caret)) {
		tokens = bytes.ReplaceAll(tokens, editor.CaretTokens, nil)
		endCaret = true
	}
	...
	if t.Context.ParseOption.EditorWYSIWYG || t.Context.ParseOption.EditorIR || t.Context.ParseOption.EditorSV || t.Context.ParseOption.ProtyleWYSIWYG {
