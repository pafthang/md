...
type Options struct {
	...
	AutoSpace bool
	// RenderListStyle 设置在渲染 OL、UL 时是否添加 data-style 属性 https://github.com/pafthang/md/issues/48
	...
	// SuperBlock 设置是否支持超级块。 https://github.com/pafthang/md/issues/111
	SuperBlock bool
	// ImageLazyLoading 设置图片懒加载时使用的图片路径，配置该字段后将启用图片懒加载。
	// 图片 src 的值会复制给新属性 data-src，然后使用该参数值作为 src 的值 https://github.com/pafthang/md/issues/55
	ImageLazyLoading string
	// ChineseParagraphBeginningSpace 设置是否使用传统中文排版“段落开头空两格”。
	ChineseParagraphBeginningSpace bool
	// Sanitize 设置是否启用 XSS 安全过滤 https://github.com/pafthang/md/issues/51
	...
	HeadingAnchor bool
	// GFMTaskListItemClass 作为 GFM 任务列表项类名，默认为 "editor-task"。
	GFMTaskListItemClass string
	// EditorCodeBlockPreview 设置 Editor 代码块是否需要渲染预览部分
	EditorCodeBlockPreview bool
	// EditorMathBlockPreview 设置 Editor 数学公式块是否需要渲染预览部分
	EditorMathBlockPreview bool
	// EditorHTMLBlockPreview 设置 Editor HTML 块是否需要渲染预览部分
	EditorHTMLBlockPreview bool
...
func NewOptions() *Options {
	return &Options{
		...
		GFMTaskListItemClass:           "editor-task",
		EditorCodeBlockPreview:         true,
		EditorMathBlockPreview:         true,
		EditorHTMLBlockPreview:         true,
		LinkBase:                       "",
		LinkPrefix:                     "",
		NodeIndexStart:                 1,
...
func (r *BaseRenderer) renderToC(node *ast.Node, entering bool) ast.WalkStatus {
	if entering {
		headings := r.headings()
		length := len(headings)
		r.WriteString("<div class=\"editor-toc\" data-block=\"0\" data-type=\"toc-block\" contenteditable=\"false\">")
		if 0 < length {
			r.WriteString("<ul>")
...
func (r *BaseRenderer) headings() (ret []*Heading) {
	...
	for _, heading := range headings {
		...
		if r.Options.EditorWYSIWYG {
			id = "wysiwyg-" + id
		} else if r.Options.EditorIR {
			id = "ir-" + id
		}
