...
func (md *MD) Md2HTML(markdown string) (sHTML string) {
	sHTML = md.MarkdownStr("", markdown)
	return
}

// SpinEditorDOM 自旋 Editor DOM，用于所见即所得模式下的编辑。
func (md *MD) SpinEditorDOM(ivHTML string) (ovHTML string) {
	ivHTML = strings.ReplaceAll(ivHTML, editor.FrontEndCaret, editor.Caret)
	markdown := md.editorDOM2Md(ivHTML)
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEditorRenderer(tree, md.RenderOptions)
	output := renderer.Render()
	ovHTML = strings.ReplaceAll(string(output), editor.Caret, editor.FrontEndCaret)
	return
}

// HTML2EditorDOM 将 HTML 转换为 Editor DOM，用于所见即所得模式下粘贴。
func (md *MD) HTML2EditorDOM(sHTML string) (vHTML string) {
	...
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEditorRenderer(tree, md.RenderOptions)
	for nodeType, rendererFunc := range md.HTML2EditorDOMRendererFuncs {
...
// EditorDOM2HTML 将 Editor DOM 转换为 HTML，用于 Editor.getHTML() 接口。
func (md *MD) EditorDOM2HTML(vhtml string) (sHTML string) {
	markdown := md.editorDOM2Md(vhtml)
	sHTML = md.Md2HTML(markdown)
	return
}

// Md2EditorDOM 将 markdown 转换为 Editor DOM，用于从源码模式切换至所见即所得模式。
func (md *MD) Md2EditorDOM(markdown string) (vHTML string) {
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEditorRenderer(tree, md.RenderOptions)
	for nodeType, rendererFunc := range md.Md2EditorDOMRendererFuncs {
		renderer.ExtRendererFuncs[nodeType] = rendererFunc
	}
	output := renderer.Render()
	vHTML = string(output)
...
// EditorDOM2Md 将 Editor DOM 转换为 markdown，用于从所见即所得模式切换至源码模式。
func (md *MD) EditorDOM2Md(htmlStr string) (markdown string) {
	htmlStr = strings.ReplaceAll(htmlStr, editor.Zwsp, "")
	markdown = md.editorDOM2Md(htmlStr)
	markdown = strings.ReplaceAll(markdown, editor.Zwsp, "")
	return
}

// RenderEChartsJSON 用于渲染 ECharts JSON 格式数据。
func (md *MD) RenderEChartsJSON(markdown string) (json string) {
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEChartsJSONRenderer(tree, md.RenderOptions)
	output := renderer.Render()
	json = string(output)
	return
}
...
func (md *MD) RenderKityMinderJSON(markdown string) (json string) {
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewKityMinderJSONRenderer(tree, md.RenderOptions)
	output := renderer.Render()
	json = string(output)
	return
}
...
func (md *MD) HTML2Md(html string) (markdown string) {
	markdown, err := md.HTML2Markdown(html)
	if nil != err {
		markdown = err.Error()
		return
	}
...
func (md *MD) editorDOM2Md(htmlStr string) (markdown string) {
	...
	tree := &parse.Tree{Name: "", Root: &ast.Node{Type: ast.NodeDocument}, Context: &parse.Context{ParseOption: md.ParseOptions}}
	tree.Context.Tip = tree.Root
	for c := htmlRoot.FirstChild; nil != c; c = c.NextSibling {
		md.genASTByEditorDOM(c, tree)
	}
...
func (md *MD) parseHTML(htmlStr string) *html.Node {
...
func (md *MD) adjustEditorDOM(root *html.Node) {
	md.removeEmptyNodes(root)
	md.removeHighlightJSSpans(root)

	for c := root.FirstChild; nil != c; c = c.NextSibling {
		md.mergeEditorDOMList0(c)
	}

	for c := root.FirstChild; nil != c; c = c.NextSibling {
		md.adjustEditorDOMListTight0(c)
	}

	for c := root.FirstChild; nil != c; c = c.NextSibling {
		md.adjustEditorDOMListList(c)
	}

	for c := root.FirstChild; nil != c; c = c.NextSibling {
		md.adjustEditorDOMListItemInP(c)
	}
}

// adjustEditorDOMListList 用于将 ul.ul 调整为 ul.li.ul。
func (md *MD) adjustEditorDOMListList(n *html.Node) {
...
func (md *MD) removeHighlightJSSpans(node *html.Node) {
	var spans []*html.Node
	for c := node; nil != c; c = c.NextSibling {
		md.hljsSpans(c, &spans)
	}
...
func (md *MD) hljsSpans(n *html.Node, spans *[]*html.Node) {
...
func (md *MD) removeEmptyNodes(node *html.Node) {
	var emptyNodes []*html.Node
	for c := node; nil != c; c = c.NextSibling {
		md.searchEmptyNodes(c, &emptyNodes)
	}
...
func (md *MD) searchEmptyNodes(n *html.Node, emptyNodes *[]*html.Node) {
	switch n.DataAtom {
	case 0:
		if md.isInline(n.PrevSibling) || md.isInline(n.NextSibling) || md.isInline(n.Parent) {
...
func (md *MD) mergeEditorDOMList0(n *html.Node) {
	switch n.DataAtom {
...
func (md *MD) adjustEditorDOMListTight0(n *html.Node) {
	switch n.DataAtom {
	case atom.Ul:
		if !md.parentIs(n, atom.Pre) {
			md.setDOMAttrValue(n, "data-tight", md.isTightList(n))
		}
	case atom.Ol:
		if !md.parentIs(n, atom.Pre) {
			md.setDOMAttrValue(n, "data-tight", md.isTightList(n))
	...
	case atom.Li:
		...
		if nil != n.PrevSibling && nil != n.FirstChild {
			...
			if "" != id && nil != n.PrevSibling.LastChild && util.DomAttrValue(n.PrevSibling.LastChild, "data-node-id") == id {
				md.setDOMAttrValue(n.FirstChild, "data-node-id", ast.NewNodeID())
			}
		}

		// 在 li 下的每个非容器块节点用 p 包裹
		for c := n.FirstChild; nil != c; c = c.NextSibling {
			if md.listItemEnter(n) {
			...
			if atom.P != c.DataAtom && atom.Blockquote != c.DataAtom && atom.Ul != c.DataAtom && atom.Ol != c.DataAtom && atom.Div != c.DataAtom {
				spans, nextBlock := md.forwardNextBlock(c)
				p := &html.Node{Type: html.ElementNode, Data: "p", DataAtom: atom.P}
				c.InsertBefore(p)
				for _, span := range spans {
					span.Unlink()
...
func (md *MD) forwardNextBlock(spanNode *html.Node) (spans []*html.Node, nextBlock *html.Node) {
...
func (md *MD) listItemEnter(li *html.Node) bool {
...
func (md *MD) isTightList(list *html.Node) string {
...
// genASTByEditorDOM 根据指定的 Editor DOM 节点 n 进行深度优先遍历并逐步生成 Markdown 语法树 tree。
func (md *MD) genASTByEditorDOM(n *html.Node, tree *parse.Tree) {
		...
		if "code-block" == dataType || "html-block" == dataType || "math-block" == dataType || "yaml-front-matter" == dataType {
			...
			for li := ol.FirstChild; nil != li; li = li.NextSibling {
				...
				if err := html.Render(originalHTML, li); nil == err {
					md := md.editorDOM2Md("<ol data-type=\"footnotes-defs-ol\">" + originalHTML.String() + "</ol>")
	...
	case 0:
		...
		if (!md.isInline(n.PrevSibling)) && strings.HasPrefix(checkIndentCodeBlock, "    ") {
	...
	case atom.Code:
		...
		if "html-inline" == dataType {
			// 所见即所得行级 HTML 解析 https://github.com/Vanessa219/editor/issues/1156
	...
	case atom.Br:
		if nil != n.Parent {
			if md.parentIs(n, atom.Td, atom.Th) {
	...
	case atom.Img:
		...
		if "emoji" == imgClass {
			...
			if "" != md.RenderOptions.LinkBase {
				src = strings.ReplaceAll(src, md.RenderOptions.LinkBase, "")
			}
			if "" != md.RenderOptions.LinkPrefix {
				src = strings.ReplaceAll(src, md.RenderOptions.LinkPrefix, "")
			}
	...
	case atom.Span:
		...
		if strings.Contains(class, "editor-comment") {
			node.Type = ast.NodeInlineHTML
			buf := bytes.Buffer{}
...
func (md *MD) hasAttr(n *html.Node, attrName string) bool {
...
func (md *MD) domChild(n *html.Node, dataAtom atom.Atom) *html.Node {
	if nil == n {
		return nil
	}

	for c := n.FirstChild; nil != c; c = c.NextSibling {
		if ret := md.domChild0(c, dataAtom); nil != ret {
			return ret
		}
	}
	return nil
...
func (md *MD) domChild0(n *html.Node, dataAtom atom.Atom) *html.Node {
...
func (md *MD) setDOMAttrValue(n *html.Node, attrName, attrVal string) {
	if nil == n {
		return
	}
...
func (md *MD) removeDOMAttr(n *html.Node, attrName string) {
	if nil == n {
		return
	}
...
func (md *MD) domCode(n *html.Node) string {
	buf := &bytes.Buffer{}
	md.domCode0(n, buf)
	return buf.String()
}

func (md *MD) domCode0(n *html.Node, buffer *bytes.Buffer) {
	if nil == n {
		return
	}
	switch n.DataAtom {
	...
	for child := n.FirstChild; nil != child; child = child.NextSibling {
		md.domCode0(child, buffer)
	}
}

func (md *MD) parentIs(n *html.Node, parentTypes ...atom.Atom) bool {
...
func (md *MD) getParent(n *html.Node, parentType atom.Atom) *html.Node {
	for p := n.Parent; nil != p; p = p.Parent {
...
func (md *MD) isCaret(n *html.Node) (isCaret, isEmptyText bool) {
	text := util.DomText(n)
...
func (md *MD) isEmptyText(n *html.Node) bool {
...
func (md *MD) startsWithNewline(n *html.Node) bool {
	return strings.HasPrefix(n.Data, "\n") || strings.HasPrefix(n.Data, editor.Zwsp+"\n")
}

func (md *MD) isInline(n *html.Node) bool {
	if nil == n {
		return false
	}
...
func (md *MD) prefixSpaces(text string) (ret string) {
	for _, c := range text {
		if ' ' == c || 160 == c {
			ret += " "
		} else {
...
func (md *MD) suffixSpaces(text string) (ret string) {
