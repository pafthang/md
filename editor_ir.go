...
// SpinEditorIRDOM 自旋 Editor Instant-Rendering DOM，用于即时渲染模式下的编辑。
func (md *MD) SpinEditorIRDOM(ivHTML string) (ovHTML string) {
	// 替换插入符
	ivHTML = strings.ReplaceAll(ivHTML, "<wbr>", editor.Caret)
	markdown := md.editorIRDOM2Md(ivHTML)
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEditorIRRenderer(tree, md.RenderOptions)
...
// HTML2EditorIRDOM 将 HTML 转换为 Editor Instant-Rendering DOM，用于即时渲染模式下粘贴。
func (md *MD) HTML2EditorIRDOM(sHTML string) (vHTML string) {
	...
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEditorIRRenderer(tree, md.RenderOptions)
	for nodeType, rendererFunc := range md.HTML2EditorIRDOMRendererFuncs {
...
// EditorIRDOM2HTML 将 Editor Instant-Rendering DOM 转换为 HTML，用于 Editor.getHTML() 接口。
func (md *MD) EditorIRDOM2HTML(vhtml string) (sHTML string) {
	markdown := md.editorIRDOM2Md(vhtml)
	sHTML = md.Md2HTML(markdown)
	return
}

// Md2EditorIRDOM 将 markdown 转换为 Editor Instant-Rendering DOM，用于从源码模式切换至即时渲染模式。
func (md *MD) Md2EditorIRDOM(markdown string) (vHTML string) {
	tree := parse.Parse("", []byte(markdown), md.ParseOptions)
	renderer := render.NewEditorIRRenderer(tree, md.RenderOptions)
	for nodeType, rendererFunc := range md.Md2EditorIRDOMRendererFuncs {
		renderer.ExtRendererFuncs[nodeType] = rendererFunc
	}
	output := renderer.Render()
	vHTML = string(output)
...
// EditorIRDOM2Md 将 Editor Instant-Rendering DOM 转换为 markdown，用于从即时渲染模式切换至源码模式。
func (md *MD) EditorIRDOM2Md(htmlStr string) (markdown string) {
	htmlStr = strings.ReplaceAll(htmlStr, editor.Zwsp, "")
	markdown = md.editorIRDOM2Md(htmlStr)
...
// genASTByEditorIRDOM 根据指定的 Editor IR DOM 节点 n 进行深度优先遍历并逐步生成 Markdown 语法树 tree。
func (md *MD) genASTByEditorIRDOM(n *html.Node, tree *parse.Tree) {
		...
		if "code-block" == dataType || "html-block" == dataType || "math-block" == dataType || "yaml-front-matter" == dataType {
			...
			for def := n.FirstChild; nil != def; def = def.NextSibling {
				...
				if err := html.Render(originalHTML, def); nil == err {
					md := md.editorIRDOM2Md(originalHTML.String())
